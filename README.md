...
本地桌面端 + 服务进程的 Codex 账号池管理器，用于统一管理账号、用量与平台 Key，并提供本地网关能力。同时支持读取和切换本机 `~/.codex` OAuth 登录态，适合把多个 Codex business / workspace 登录记录统一收进一个工作台里管理。
...
## 功能概览
- 账号池管理：分组、标签、排序、备注、启用 / 禁用、优先账号
- 批量导入 / 导出：支持多文件导入、桌面端文件夹递归导入 JSON、按账号导出单文件
- 用量展示：兼容 5 小时 + 7 日双窗口，以及仅返回 7 日单窗口的账号
- 授权登录：浏览器授权 + 手动回调解析，支持按 `workspaceId / org-...` 记录不同 business 登录态
- 本地 Codex 工作台：扫描本机 `~/.codex`、导入当前登录态、切换本地 OAuth token、展示已信任项目
...
4. 刷新用量并确认账号状态。
5. 如需管理本机 Codex 登录态，进入“本地 Codex”页面扫描并导入当前本机登录。
6. 如需让 IDE / 第三方工具走账号池和请求日志，改用 CodexManager 的本地网关地址与平台 Key。

## 本地 Codex 工作台
- 独立入口：桌面端侧边栏提供单独的“本地 Codex”页面，不再放在系统设置中。
- 当前登录态：显示当前写入本机 `~/.codex/auth.json` 的 workspace、account 和本地目录状态。
- 工作空间切换：支持把已经保存在 CodexManager 中的不同 Codex OAuth 登录态一键写回本机 `auth.json`。
- 已信任项目：读取 `~/.codex/config.toml` 中的项目信任范围，用于辅助确认当前本机 Codex 的工作目录上下文。

说明：
- 这里切换的是“本机 Codex OAuth 登录态”，不是网关路由优先账号。
- 同一 `workspace_id` 下允许保留多条不同 token 记录，便于在多个 business 登录态之间切换。
- 仅靠 `~/.codex` 当前文件无法自动恢复你历史上所有登录过的 business；需要至少导入过一次，CodexManager 才能记住并提供后续一键切换。

## 网关与请求日志
- CodexManager 不只是账号面板，还可以作为本地 OpenAI 兼容网关。
- 只有真正经过 CodexManager 本地监听端口的请求，才会进入“请求日志”页面。
- 在官方 Codex CLI、本机 `~/.codex` 登录态、ChatGPT / Codex 网页里直接发起的请求，不会自动出现在请求日志中。
- 如果你希望 IDEA、CLI 或其他工具走账号池、优先账号、失败回退和请求日志，需要把这些工具的 `Base URL` 指到 CodexManager 本地地址，并使用平台 Key。

## 页面展示
### 桌面端
- 账号管理：集中导入、导出、刷新账号与用量
- 本地 Codex：扫描本机登录态、切换本地 OAuth 工作空间 / business、查看已信任项目
